...
function Card({ asset, index }: { asset: IASA, index: number }) {
  ...
  return (
    ...
    >
      <Avatar size='sm' display="block" m=".5em auto" name={asset.name} src={asset.logo} />
      <Text textAlign="center" my={"1em"}>
        {asset.name.trim().length >= 20 ? `${asset.name.substring(0, 15)}...` : asset.name  }
      </Text>
      <Button display="block" margin=".3em auto" bgColor={ btnVariantColor } _hover={{ backgroundColor: btnVariantColor}} borderRadius="8px">
